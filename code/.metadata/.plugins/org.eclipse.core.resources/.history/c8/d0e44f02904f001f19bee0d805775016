...
int main(void)
{
  ...
  MX_USB_DEVICE_Init();
  /* USER CODE BEGIN 2 */
  HAL_TIM_Encoder_Start_IT(&htim2, TIM_CHANNEL_ALL); //Encoder timer
  HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_1); //MOSFET control PWM timer
  TIM1->CCR1 = 0;
  ...
  usbCom = new UsbManager(myMenu);
  //myMenu->hwInterface->calC(500);
  int updateInt = 0;
  while (1)
  {
	  btn = !HAL_GPIO_ReadPin(ENC_BTN_P, ENC_BTN);
	...
	if(runMenu && !myMenu->selecting())
	{
		//myMenu->prevBtn = btn;
		myMenu->run();
	}

	myMenu->updateHardware();
	if((updateInt == 10)) //&& !btn)
		{
			//myMenu->updateHardware();
			myMenu->hwInterface->readCFiltered();
			myMenu->hwInterface->readVoltageFiltered();
			myMenu->hwInterface->readW();
			updateInt = 0;
		}
